...
img_index = [4];
img_left_matrix = zeros(288,288,3,length(img_index));
img_right_matrix = zeros(288,288,3,length(img_index));
img_third_matrix = zeros(288,288,3,length(img_index));
for i = img_index
    xx = 1;
    img_left = imread([img_dir,'im',num2str(2*i-1),'.png']);
    img_left_matrix(:,:,:,xx) = img_left;
    img_right = imread([img_dir,'im',num2str(2*i),'.png']);
